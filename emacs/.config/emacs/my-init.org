...
** ~csharp.el~
*** ~csharp-mode~
C# support for Godot and .NET development. ~csharp-ts-mode~ is built-in
to Emacs 29+; ~lsp-csharp~ (OmniSharp Roslyn) ships with ~lsp-mode~.

**** use-package
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-csharp.el" :mkdirp yes
