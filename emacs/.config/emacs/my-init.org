...
** disable toolbar before frame creation
#+begin_src emacs-lisp :tangle "early-init.el" :mkdirp yes
  (tool-bar-mode -1)
  ;;(push '(tool-bar-lines . 0) default-frame-alist)
#+end_src

** silence harmless async native-comp warnings
Third-party packages (elpaca builds) routinely trip "not known to be
defined" warnings for functions guarded by fboundp/version checks, or
forward-referenced later in the same file. These are compile-time
noise, not runtime bugs, and popping up a warnings buffer for them on
every async compile is more distracting than useful.
#+begin_src emacs-lisp :tangle "early-init.el" :mkdirp yes
...
*** Remove blocked ELPA menu sources
=elpa.gnu.org= and =elpa.nongnu.org= are blocked on the corporate network.
Packages exclusively on GNU/NonGNU ELPA have explicit =:ensure= recipes in their modules.
#+begin_src emacs-lisp :tangle "init.el" :mkdirp yes
...
* Modules
** ~miscellaneous.el~
Also import NODE_EXTRA_CA_CERTS so Node-based servers (Copilot, ts-ls, angular-ls) trust the corporate CA bundle (~/.certs/ca-bundle.pem)
...
** ~godot.el~
*** ~gdscript-mode.el~
This is the official package managed by Godot

**** use-package
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-godot.el" :mkdirp yes
...
*** use and config denote-sequence
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-denote.el" :mkdirp yes
...
Copy ~file:line~ to clipboard, relative to project root.
Works in regular buffers (includes line number) and dired (path only).
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-tweakers.el" :mkdirp yes
...
Opens the current buffer's file (or the file at point in dired) in Google
Chrome. Useful for previewing Markdown via the browser's Markdown extension.
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-tweakers.el" :mkdirp yes
(defun my/open-in-chrome ()
  "Open the current buffer's file in Google Chrome."
  (interactive)
  (let ((file (or (buffer-file-name)
                  (and (derived-mode-p 'dired-mode) (dired-get-file-for-visit)))))
    (unless file (user-error "Buffer is not visiting a file"))
    (call-process "open" nil 0 nil "-a" "Google Chrome" file)))

(global-set-key (kbd "C-c b") #'my/open-in-chrome)
#+end_src
...
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-blamer.el" :mkdirp yes :prologue ";;; -*- lexical-binding: t; -*-"
(use-package
 blamer
...
-macOS dependencies:
#+begin_src sh
  brew install enchant hunspell pkg-config
#+end_src
...
System dependencies — install the ACP bridges:
#+begin_src sh
...
*** use and config mermaid-mode
#+begin_src emacs-lisp :tangle "tony-emacs-modules/tony-emacs-mermaid.el" :mkdirp yes
