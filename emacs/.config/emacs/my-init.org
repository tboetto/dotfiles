...
*** Claude daily cost tracking
Reads today's cumulative cost from ~ccusage~ (which parses =~/.claude/projects/*/= JSONL
files) and displays it in the global mode-line as =​$X.XX/d=.  Updated asynchronously
after every agent turn via the =turn-complete= event.

Requires ~ccusage~ to be available via =npx=:
#+begin_src sh
